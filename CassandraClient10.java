...
public class CassandraClient10 extends DB
{
  ...
  public int read(String table, String key, Set<String> fields, HashMap<String, ByteIterator> result)
  {
    ...
    for (int i = 0; i < OperationRetries; i++)
    {
      ...
      try
      {
        ...
        //  Client.fwriter.println("inside read - key: " + key + " name:" +name+" value:"+value + " timestamp:"+ column.getTimestamp());
         // Client.fwriter.flush();
          
            
          if (maxTimestamp < column.getTimestamp())        
            maxTimestamp = column.getTimestamp();
        ...
        writeFile("D\tR\t" + key + "\t" + maxTimestamp+"\t"+System.currentTimeMillis()+"\t"+Thread.currentThread().getId());
        
        Long t1, t2;        
        // Monotonic read consistency
        
         t1 = Client.readTimes.get(key); //Will hold timestamp obtained at last read time
          /*
          Client.fsmall.println("before checking -  readTime and maxTimestamp: " + t1 + " " + maxTimestamp);
          Client.fsmall.flush();       
         */
          // Measure monotonic read consistency
          if (t1 != null && t1 > maxTimestamp) {
              Client.rd_violations.getAndIncrement();
            //  Client.fsmall.println("inside read - key: " + key );
        //  Client.fsmall.println("inside mr -  readTime and maxTimestamp: " + t1 + " " + maxTimestamp);
        //  Client.fsmall.flush();
          }
          else {
          // Put timestamp obtained on latest read into hashtable
          Client.readTimes.put(key,maxTimestamp);
          }     
                  
          /*
          Client.fsmall.println("after mr, before raw - readTime and maxTimestamp: " + t1 + " " + maxTimestamp);
          Client.fsmall.flush();       
         */
          
          t1 = Client.readTimes.get(key);
          t2 = Client.writeTimes.get(key);

         /*
          Client.fsmall.println("inside read - key: " + key + " timestamp:"+ maxTimestamp);
          Client.fsmall.flush();
          */
          
          
          // Measure Read your Write consistency
          if (t1 != null && t2 != null) {
          if (t2 > t1) {
              Client.rw_violations.getAndIncrement();
              //Client.fsmall.println("inside read - key: " + key );
              //Client.fsmall.println("inside rw - writeTime and readTime and maxTimestamp: " + t2 + " "+ t1 + " " + maxTimestamp);
          //Client.fsmall.flush();       
    ...
    for (int i = 0; i < OperationRetries; i++)
    {
      ...
      try
      {
        ...
        for (Map.Entry<String, ByteIterator> entry : values.entrySet())
        {
          ...
          long filledTime = System.currentTimeMillis();
          col.setTimestamp(filledTime);
                    
          // Put into hashtable
          Client.writeTimes.put(key,filledTime);
          
          writeFile("D\tW\t" + key + "\t" + filledTime+"\t"+System.currentTimeMillis()+"\t"+Thread.currentThread().getId());
          
         // writeFile("[I]\tkey:" + key + "\t" + filledTime+"\t"+System.currentTimeMillis());
          
        //  Client.fwriter.println("inside insert. key:" + key + " name:" + col.name + " value:"+ col.value+ " timestamp:" + filledTime);
        //  Client.fwriter.flush();
  ...
  synchronized public static void writeFile(String x) {
      Client.fsmall.println(x);
      Client.fsmall.flush();
